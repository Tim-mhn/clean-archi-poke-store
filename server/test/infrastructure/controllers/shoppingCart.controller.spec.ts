...
describe('ShoppingCartController @Post :shoppingCartId/pokemon/:pokemonId endpoint', () => {
    beforeEach(() => {
        ...
        pokeRepo = Sinon.createStubInstance(
...
describe('Shopping Cart Controller - @Get ShoppingCartDetails (content, price, estimatedReadyDate', () => {
    ...
    it('should call usecase execute function and presenter present function is no errors are thrown', async () => {
        ...
        expect(
            controller.getShoppingCartContentPriceAndReadyDateUseCase.execute
        ).to.be.calledOnce
        expect(
            controller.getShoppingCartContentPriceAndReadyDatePresenter.present
        ).to.be.calledOnce
    })
    ...
    it("should call presenter's formatOnError if it catches an error in try block", async () => {
        ...
        expect(
            controller.getShoppingCartContentPriceAndReadyDatePresenter
                .presentOnError
        ).to.be.calledOnce
        expect(res.status).to.be.calledOnceWithExactly(
            presenterOnErrorOutput[0]
        )
